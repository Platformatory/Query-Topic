...
def consume_messages(start_time, end_time, string_input, config, topic, csv_file=None):
    ...
    if csv_file:
        ...
        csv_writer.writerow(['timestamp', 'message'])  # Adjust based on your needs
    ...
    for partition_offset in offsets:
        ...
        if partition_offset.offset != -1:
            ...
            while True:
                ...
                msg_timestamp = msg.timestamp()[1]  # Get timestamp in milliseconds

                # Stop if the message's timestamp is beyond 10 minutes from the start
                if msg_timestamp > end_timestamp:
                    #print(f"Stopping partition {partition_offset.partition}, reached 10 minutes window.")
                    break
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start_time', type=str, required=True, help="Start time in 'YYYY-MM-DD HH:MM:SS' format (optional)")
    parser.add_argument('--end_time', type=str, required=True, help="End time in 'YYYY-MM-DD HH:MM:SS' format (optional)")
